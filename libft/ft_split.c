...
/*   Updated: 2025/06/30 13:32:44 by lpalomin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**split_process(char const *s, char c, char **str_group)
{
	size_t	count;
	...
	while (*s)
	{
		...
		if (*s == '\0')
			break ;
		if (!add_word(&s, c, str_group, &count))
		{
			while (count--)
				free(str_group[count]);
			free(str_group);
			return (NULL);
		}
	}
	str_group[count] = NULL;
	return (str_group);
}
...
char	**ft_split(char const *s, char c)
{
	...
	if (!str_group)
		return (NULL);
	return (split_process(s, c, str_group));
}
