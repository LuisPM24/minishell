...
/*   Updated: 2025/09/26 17:46:27 by mmembril         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void							add_to_cmd(t_cmd *cmd, char *new_line,
									int *count2);
...
// dollar_utils.c
char							*expand_dollar_line(t_cmd *cmd, char *line,
									int position, char **envp);
char							*get_dollar_value(t_cmd *cmd, char *line,
									int start, char **envp);
int								search_dollars(char *line);
// more_dollar_utils.c
void							expand_dollars(t_cmd *cmd, char **envp);
void							remove_quotes_pipe_argv(t_cmd *cmd);
char							*expand_all_dollars(t_cmd *cmd, char *line,
									char **envp);
...
void							add_redir(t_redir **list, t_redir_type type,
									char *s);
...
void							free_dollar_line(char *full_var,
									char *dollar_value, char *prefix,
									char *suffix);
// pipes_utils.c
...
// fork_utils.c
int								fork_and_exec(t_cmd *cmd, char **envp,
									int pipe_cmd, int *fds);
// execute_builtins.c
...
int								builtin_export(t_cmd *cmd, int pipe_cmd,
									char **envp);
int								builtin_unset(t_cmd *cmd, int pipe_cmd,
									char **envp);
...
void							remove_argv_range(t_cmd *cmd, int pipe_cmd,
									int start, int count);
